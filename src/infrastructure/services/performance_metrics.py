...
class PerformanceMetrics:
    ...
    def get_all_stats(self) -> dict:
        """Получить статистику по всем этапам."""
        return {
    ...
    def estimate_duration(self, stage: str, default: float = 5.0) -> float:
        """Оценить время выполнения этапа.
        ...
        if metrics_file.exists():
            try:
                ...
                for stage_data in data.get("stages", []):
                    stage = StageMetrics(name=stage_data["name"])
                    stage.samples = stage_data.get("samples", [])
        ...
        try:
            data = {
                "stages": [
                    {
                        "name": s.name,
                        "samples": s.samples[-20:],  # Последние 20
                    }
