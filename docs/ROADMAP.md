...
**Возможности проекта уже превосходят то, что текущие локальные модели стабильно раскрывают.** Приоритет — **полировка бэкенда**: архитектура, качество кода, тесты, стабильность, производительность. Когда появятся более сильные локальные LLM, продукт будет готов к ним без переделок.

- **Сейчас:** бэкенд — вылизать API, рефакторинг, покрытие, логи, безопасность; фронт — минимальная полировка UX (Escape, подсказки).
...
Чеклист по полировке бэкенда: [docs/BACKEND_POLISH.md](BACKEND_POLISH.md).
...
| **Полировка UX** | Большая часть уже есть. Осталось: Escape для панели «Настройки» и модалки Git diff (см. «Улучшения UI»). | **Низкий** |
...
**Итог:** По возможностям (чат, агент, multi-file, apply/reject, RAG, анализ) TAI уже близок к Cursor. Сейчас фокус — полировка бэкенда ([BACKEND_POLISH.md](BACKEND_POLISH.md)); правила проекта и инлайн-подсказки — когда появятся топовые локальные LLM.
...
| Что | Где проверить | Зачем |
|-----|----------------|--------|
| **Escape для панели «Настройки»** | `Layout.tsx`, `SettingsPanel.tsx` | Сейчас закрытие только по кнопке «Закрыть». Добавить `keydown` Escape → `setShowSettings(false)`. |
| **Escape для модалки Git diff** | `GitPanel.tsx` — `selectedDiff` | Сейчас закрытие по клику на overlay или кнопку. Добавить `keydown` Escape → `setSelectedDiff(null)`. |

### 2. Производительность — осталось

| Что | Где | Зачем |
|-----|-----|--------|
| **Прогрев кэша моделей при старте** | `main.py` lifespan, `ModelSelector` | Сейчас при первом запросе в чат вызывается `list_models` (лишний round-trip). В lifespan после `validate_models_config` один раз вызвать прогрев кэша селектора моделей (если есть доступ к контейнеру/LLM), чтобы первый запрос не ждал. |
| **CACHE_TTL для списка моделей** | `model_selector.py` — `CACHE_TTL = 60` | Опционально: для single-user поднять до 2–5 мин, чтобы реже сбрасывать кэш. |
| **Стриминг** | — | Уже есть для чата, improve, workflow; при необходимости пройти сценарии и убедиться, что нет «долгой генерации без стрима». |
...
| Что | Зачем |
|-----|--------|
| **Разбить project_analyzer на модули** | Часть 5 ROADMAP — проще тестировать и дорабатывать анализ. |
| **Вынести логику из больших файлов** | Тот же план — уменьшить когнитивную нагрузку. |
| **Быстрый CI: `pytest -m "not slow"`** | Зафиксировать в CONTRIBUTING или CI. |

### 5. Приоритет внедрения (обновлённый)

1. **Быстро:** Escape для Настроек и Git diff (два небольших дополнения).
2. **По желанию:** прогрев кэша моделей в lifespan; увеличение CACHE_TTL; подсказка для режима Агент.
3. **Далее:** рефакторинг project_analyzer и больших файлов по мере необходимости.
...
**Дальше по приоритету (фокус — полировка бэкенда):**

1. ~~**A4** Покрытие тестами в анализе~~ — ✅ сделано.
2. ~~**B6** Inline-редактирование~~ — ✅ сделано.
3. ~~**C3** Стриминг улучшений~~ — ✅ сделано.
4. **Полировка бэкенда:** см. [BACKEND_POLISH.md](BACKEND_POLISH.md) — проверка DI, project_analyzer, большие файлы, безопасность (FileService._is_safe_path), тесты без Ollama, прогрев кэша моделей.
...
**Осталось:** Escape для панели «Настройки» (`Layout`/`SettingsPanel`) и для модалки Git diff (`GitPanel`). См. раздел «Полировка и оптимизация» выше.
