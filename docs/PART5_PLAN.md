...
Использовать этот документ как чеклист; отмечать выполненное по мере работы.
...
- [ ] Убедиться, что в коде не осталось обращений к старым глобальным синглтонам (кроме _container в container.py).
- [ ] Обновить ROADMAP.md: в Части 5 отметить «Глобальные переменные → DI» как выполненное.
...
- [ ] **3.3.1** Вынести форматирование результатов в функцию format_search_results или отдельный модуль formatters.
- [ ] **3.3.2** Вынести построение URL/заголовков для каждого провайдера (SearXNG, Brave, Tavily, Google) в маленькие функции или классы-стратегии.
- [ ] **3.3.3** Оставить в основном файле оркестрацию: multi_search, выбор провайдера, таймауты.
...
- [ ] Тесты для затронутых областей (improvement, analysis, web_search при наличии).
- [ ] В ROADMAP отметить «Большие файлы: вынести логику».
