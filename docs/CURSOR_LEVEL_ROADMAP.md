...
|---------|-------------|---------|
| **Анализ** | DeepAnalyzer: ключевые файлы, RAG 8 запросов, фреймворк, промпты | Многошаговость, граф зависимостей, Git |
...
- [ ] Шаг 1: LLM получает статику + ключевые файлы → выделяет 3–5 проблемных модулей
- [ ] Шаг 2: RAG по каждому модулю (targeted search) → углублённый контекст
- [ ] Шаг 3: LLM финальный синтез с приоритетами
- [ ] Fallback: если токены/время — один проход как сейчас
...
- [ ] При improve_file: RAG search по file_path + issue
- [ ] Добавить в plan/code промпт: релевантные чанки (вызовы, аналоги)
- [ ] Учёт: как этот файл вызывается, какие паттерны в проекте

### Phase B2: Project map в генерацию кода (приоритет: высокий)
- [ ] Workflow coder: получает project_map (структура, классы, функции)
- [ ] Improvement: получает project_map для контекста стиля
- [ ] Промпт: "Следуй структуре проекта: ..."
...
- [ ] При отправке сообщения: автоматический RAG search по запросу
- [ ] Инжектировать топ-5 чанков в контекст (если не @rag явно)
- [ ] Настраиваемо: вкл/выкл в настройках
...
- [ ] При старте: автоматически search_rag по запросу пользователя
- [ ] Инжектировать project_map в system prompt (первые 2K символов)
- [ ] read_file: при чтении — предлагать связанные (по импортам)
