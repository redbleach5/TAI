# Руководство по проверке проекта TAI
...
## 8. Advanced IDE (Phase 8)

### 8.1 File Browser

**Зачем:** Навигация по файлам проекта без выхода из интерфейса.

**Как:**
1. Перейти во вкладку IDE
2. В левом сайдбаре — дерево файлов
3. Двойной клик открывает файл в редакторе
4. ПКМ — контекстное меню (New, Rename, Delete)

**Ожидаемо:**
- Дерево показывает структуру проекта
- `__pycache__`, `.git`, `node_modules` скрыты
- Git-статус файлов (M, A, ?) отображается

- [v] Дерево файлов загружается
- [v] Файлы открываются по двойному клику
- [v] Контекстное меню работает
- [v] Git-статус отображается

### 8.2 Multi-File Editor

**Зачем:** Редактирование нескольких файлов с табами.

**Как:**
1. Открыть несколько файлов из File Browser
2. Переключаться между табами
3. Редактировать и сохранять (Ctrl+S)

**Ожидаемо:**
- Табы открытых файлов
- Индикатор несохранённых изменений
- Ctrl+S сохраняет файл
- Ctrl+W закрывает таб

- [v] Табы файлов работают
- [v] Редактирование и сохранение
- [v] Закрытие табов (крестик, middle-click, Ctrl+W)
- [v] Monaco Editor с подсветкой синтаксиса

### 8.3 Terminal

**Зачем:** Выполнение команд без переключения окон.

**Как:**
1. В режиме IDE — терминал снизу
2. Кликнуть для раскрытия
3. Ввести команду: `python --version`, `ls`, `npm run build`

**Ожидаемо:**
- Команды выполняются
- История команд (стрелки вверх/вниз)
- Безопасные команды работают (python, npm, git, ls)
- Опасные команды блокируются (curl, rm -rf /)

**Как проверить:**
```bash
curl -X POST http://localhost:8000/terminal/exec \
  -H "Content-Type: application/json" \
  -d '{"command": "python --version"}'
```

- [v] Terminal UI работает
- [v] Команды выполняются
- [v] История команд
- [v] Блокировка опасных команд

### 8.4 Git Panel

**Зачем:** Source control без выхода из интерфейса.

**Как:**
1. В режиме IDE переключить сайдбар на Git (иконка)
2. Увидеть изменённые файлы
3. Ввести commit message и нажать Commit

**Ожидаемо:**
- Текущая ветка отображается
- Список изменённых файлов (M, A, ?)
- Diff при клике на файл
- Коммит работает

**Как проверить:**
```bash
curl http://localhost:8000/git/status
curl "http://localhost:8000/git/diff?path=README.md"
```

- [v] Git status отображается
- [v] Diff preview
- [v] Commit работает
- [v] История коммитов
